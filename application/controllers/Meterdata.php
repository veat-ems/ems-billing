...
class Meterdata extends CI_Controller {
    ...
    public function edit($id_meter, $random=false) {
		...
		if($this->form_validation->run() != false){
			...
			$nmfile = $metergroupid; //"front_".$id_meter; //nama file saya beri nama langsung dan diikuti fungsi time
			...
			if($_FILES['frontpicture']['name'])
			{
			 	if ($this->upload->do_upload('frontpicture'))
				{
					...
					// update to all meter in group == All member with same picture
					$this->db->where('metergroupid', $metergroupid);
					$this->db->update('data_meter',$dataf2);
				}
				else{
