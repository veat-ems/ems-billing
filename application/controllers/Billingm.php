...
class Billingm extends CI_Controller
{
	...
	public function index()
	{
		...
		$dari = date('Y-m-d');
		$dari 	= date('Y-m-01', strtotime($dari. ' -1 month'));

		$sampai = date('Y-m-d');
		$sampai = date('Y-m-01', strtotime($sampai));
		// $sampai = date('Y-m-t', strtotime($dari));

		// $startDate = date('Y-m-d'); // sekarang
		// $dari = date('Y-m-d', strtotime($startDate . ' -1 month'));
		// $sampai = date('Y-m-d', strtotime($startDate . ' +0 month'));


		$dari_time = '00:00:00';
		$sampai_time = '00:00:00'; //'23:59:59';
		...
		// var_dump($dari_time);
		// var_dump($sampai);
		// var_dump($sampai_time);
		// die();

		$this->template->display_trend('billing_formm', $d);
	}
	...
	public function calculation()
	{
		...
		$this->template->display_table('billingm', $d);
	...
	public function pdf()
	{
		...
		$tarif_lwbp 	= $this->input->post('tarif_lwbp');
		$tarif 	 		= $this->input->post('tarif');
		$kurs	 		= $this->input->post('kurs'); // untuk BEA ADMIN
		$dari 	 		= $this->input->post('dari');
		//test
		// $dari = '2020-12-01';
		$dari_time 	 	= $this->input->post('dari_time');
		...
		$var_date_from	= $dari . ' ' . $dari_time;
		$var_date_thru	= $sampai . ' ' . $sampai_time;
		$tbl_name 		= 'pg_counter_min'; //'pg_counter_hour';
		$list 			= $this->model_billing->usagereport($var_date_from, $var_date_thru, $str_meters, $tbl_name)->result();
		$data 			= array();
		...
		foreach ($list as $billing) {
			...
			$row['lokasi']	 		= $billing->lokasi;
			$metergroup_id 			= $billing->metergroupid;

			$table 					= 'metergroups';

			$this->db->select("metergroupname")
				->from($table)
				->where("metergroupid", $metergroup_id);
			$query 	= $this->db->get();
			// var_dump($query->num_rows());
			// die;
			if ($query->num_rows() == 1) {
				$row['metergroupname'] = $query->row('metergroupname');
			} else {
				$row['metergroupname'] = '';
			}
			// $mgn = $query->row('metergroupname');





			$row['daya']	 		= number_format($billing->p_nominal / 1, 0);
			$bill_date				= strtotime($billing->date_time_stop);

			if ($bill_date) {
				$row['bill_month'] 		= date('d M Y', $bill_date);
			} else {
				$row['bill_month'] 		= '-';
			}
			...
			$row['kwh_usage'] 		= number_format($billing->kwh_exp_usage / 1000, 1);
			...
			$row['kwh1_usage'] 		= number_format($billing->kwh1_usage / 1000, 1);
			...
			$row['kwh2_usage'] 		= number_format($billing->kwh2_usage / 1000, 1);
			$kwh2_tot				= ($billing->kwh2_usage / 1000) * $tarif_lwbp;
			$row['kwh2_total'] 		= number_format($kwh2_tot, 0);

			$kwh12_totalrp	    	= $kwh1_tot + $kwh2_tot;
			$row['kwh1_totalrpF'] 	= number_format($kwh1_tot, 0);
			$row['kwh2_totalrpF'] 	= number_format($kwh2_tot, 0);
			$row['kwh12_totalrpF'] 	= number_format($kwh12_totalrp, 0);


			$adminrp				= $kurs;
			$row['adminrpF'] 		= number_format($adminrp, 0);
			...
			$row['kwh12_admin_ppn_totalrpF']	= number_format($kwh12_admin_ppn_totalrp, 0);
		...
		$html = $this->load->view('invoicem', $d, true);
		// $namafile = 'Electricity_Usage_Report_' . date('d-m-Y_H-i-s');
		$namafile = 'Electricity_Usage_Report_' . date('Ymd_His');
		// $this->pdf->createPDF($html, $namafile);
		$this->pdf->generate($html, $namafile, 'A4', 'portrait', true);
		//true -> Preview open new tab ; false -> save as
	...
	public function xls()
	{
		...
		foreach ($list as $billing) {
			...
			$kwh12_totalrp	    	= $kwh1_tot + $kwh2_tot;
			$row['kwh12_totalrpF'] 	= number_format($kwh12_totalrp, 0);
			$kwh12_ppnrp			= $kwh12_totalrp * 0.11;
			$row['kwh12_ppnrpF'] 	= number_format($kwh12_ppnrp, 0);
		...
		// var_dump($d);
		// die();
		$this->load->view('billing_excelm', $d);
	}
}
